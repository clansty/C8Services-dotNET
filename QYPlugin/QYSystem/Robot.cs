...
namespace QYPlugin
{
    ...
    public static class Robot
    {
        private static class DllEvents
        {
            ...
            {
                var ifo = InfoParse(fromInfo);

                var u = new Unpack(ifo["GExtraInfo"]);
                var nick = u.NextStr;
                var gc = u.NextStr;
                u.Skip(12);
                var title = u.NextStr;
        ...
        private static long LongQQ
        {
            get
            {
                var base64 = Marshal.PtrToStringAnsi(QY_getLoginQQList(AuthCode));
                var u = new Unpack(base64);
                long qq = 0;
                if (u.NextInt > 0)
                    qq = u.NextLong;
        ...
        public static class Send
        {
            ...
            {
                var res = long.TryParse(qq, out var targ);
                if (!res)
                    return false;
                var ret = QY_sendFriendMsg(AuthCode, LongQQ, targ, msg);
                return ret == 0;
            }
            [DllImport("QYOffer.dll")]
            ...
            {
                var res = long.TryParse(group, out var targ);
                if (!res)
                    return false;
                var ret = QY_sendGroupMsg(AuthCode, LongQQ, targ, msg);
                return ret == 0;
            }
            [DllImport("QYOffer.dll")]
            ...
            public static bool GTmp(string group, string qq, string msg)
            {
                ...
                if (!res)
                    return false;
                res = long.TryParse(qq, out var targqq);
                if (!res)
                    return false;
                var ret = QY_sendGroupTmpMsg(AuthCode, LongQQ, targ, targqq, msg);
                return ret == 0;
            }
            [DllImport("QYOffer.dll")]
        ...
        {
            var isok = long.TryParse(target, out var t);
            if (!isok)
                return LikeResult.notQQ;
            var res = QY_sendLikeFavorite(AuthCode, LongQQ, t);
            switch (res)
            {
                case 0:
        ...
        public static class Group
        {
            ...
            {
                var res = long.TryParse(group, out var t);
                if (!res)
                    return false;
            ...
            public static string GetCard(string group, string member)
            {
                ...
                if (!isok)
                    return "";
                var b64 = Marshal.PtrToStringAnsi(QY_getGroupMemberCard(AuthCode, LongQQ, t, m, false));
            ...
            public static GroupInfo GetInfo(string group)
            {
                ...
                var b64 = Marshal.PtrToStringAnsi(QY_getGroupInfo(AuthCode, LongQQ, t));
                var u = new Unpack(b64);
                var a = u.NextLong;
                var b = u.NextLong;
                u.Skip(4);
            ...
            {
                long.TryParse(group, out var t);
                var b64 = Marshal.PtrToStringAnsi(QY_getGroupMemberList(AuthCode, LongQQ, t));
                var u = new Unpack(b64);
                var count = u.NextInt;
        ...
        {
            var res = long.TryParse(target, out var targ);
            if (!res)
                return false;
